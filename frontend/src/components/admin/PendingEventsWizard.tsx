...
interface PendingEventsWizardProps {
    venues: Venue[];
    categories: Category[];
    organizers?: any[];
}

const STORAGE_KEY = 'pending_events_wizard_session';

export const PendingEventsWizard: React.FC<PendingEventsWizardProps> = ({ venues, categories, organizers = [] }) => {
    ...
    const fetchPendingEvents = async () => {
        ...
        try {
            ...
            if (pendingList.length === 0) {
                setParseError("Queue clean! No pending events to review.");
                setStagedEvents([]);
                return;
            }

            const staged: StagedEvent[] = pendingList.map((item: any) => {
                ...
                return {
                    ...
                    min_age: parseInt(item.age_restriction || '0') || 0,
                    ...
                    location_name: item.location_name || item.venue_name || item.venue || 'Unknown Location',
            ...
            setParseError(`Failed to fetch pending events: ${err.message}`);
            setStagedEvents([]);
        } finally {
            setIsFetching(false);
        }
    ...
    const rejectAndNext = async () => {
        ...
        try {
            await adminAPI.rejectPendingEvent(event.id);
            setStagedEvents(prev => {
                const next = [...prev];
            ...
            setImportError(`Failed to reject event: ${err.message}`);
        }
    };
    ...
    const approveAndImport = async () => {
        ...
        try {
            const selectedCatName = categories.find(c => c.id === event.selectedCategoryId)?.name;
            const payload = {
                title: event.title,
                ...
                age_restriction: String(event.min_age || 0),
                venue_id: event.selectedVenueId || undefined,
                venue_name: event.location_name,
                category_name: selectedCatName || undefined
            };

            await adminAPI.approvePendingEvent(event.id, payload);
            ...
            if (err?.message?.includes('Duplicate event') || err?.detail?.includes('Duplicate event')) {
                setStagedEvents(prev => {
                    const next = [...prev];
                    next[currentReviewIndex] = { ...next[currentReviewIndex], status: 'duplicate', message: 'Duplicate detected. Event already exists in database.' };
                    return next;
                });
                setImportError('Duplicate detected. Verify manually if this is a new event.');
            } else {
                setStagedEvents(prev => {
                    const next = [...prev];
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {!showResumePrompt && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div className="bg-emerald-50/50 p-4 rounded-lg border border-emerald-100 shadow-sm">
                        <label className="block text-sm font-medium text-emerald-800 mb-2">
                            1. Load Scraped/Queued Events
                        </label>
                        <button
                            onClick={fetchPendingEvents}
                            disabled={isFetching}
                            className="w-full px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 font-bold"
                        >
                            {isFetching ? 'Loading queue...' : 'Fetch Pending Events'}
                        </button>
