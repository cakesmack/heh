...
export function EventFilters({ onFilterChange, userLocation, initialFilters, isCollectionMode = false }: EventFiltersProps) {
  // Categories from API
  ...
  const wrapInMode = (children: React.ReactNode) => {
    if (isCollectionMode) {
      return (
  ...
  return (
    <div className={isCollectionMode ? "" : "bg-white p-4 rounded-lg border border-gray-200"}>
      {!isCollectionMode && (
      ...
      {wrapInMode(
        <>
          ...
          {/* Location Filter */}
          <div className={isCollectionMode ? "w-full md:w-auto md:flex-1" : ""}>
            {!isCollectionMode && (
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Location
              </label>
              ...
              <div className="flex-1">
                <LocationInput
                  onSelect={(loc) => {
                    ...
                    setLocationInput(loc.placeName);
                  }}
                  onChange={(val) => {
                    setLocationInput(val);
                    // Clear the coordinate selection if the text no longer matches the selected place
                    if (selectedLocation && val !== selectedLocation.placeName) {
                      setSelectedLocation(null);
                    }
                  }}
                  onKeyDown={(e) => {
                  ...
                  hideAttribution={isCollectionMode}
                />
              </div>
              <button
                type="button"
                ...
                className="flex items-center justify-center px-3 h-11 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 hover:bg-emerald-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  ...
                  <div className="flex gap-1">
                    {[10, 20, 50].map((radius) => (
                      <button
                        ...
                        className={`px-2 py-1 text-xs rounded-full border transition-colors ${selectedRadius === radius
                          ? 'bg-emerald-100 border-emerald-500 text-emerald-800'
          ...
          <div className={isCollectionMode ? "w-full md:w-auto md:flex-1" : ""}>
            ...
            <SingleDateRangePicker
              dateFrom={dateFrom ? parseISO(dateFrom) : null}
              dateTo={dateTo ? parseISO(dateTo) : null}
          ...
          {/* Action Buttons */}
          <div className={`flex gap-2 ${isCollectionMode ? "flex-row w-full md:w-auto mt-2 md:mt-0" : "flex-row pt-2"}`}>
            <Button
              onClick={handleApplyFilters}
              variant="primary"
              size="sm"
              fullWidth={!isCollectionMode}
              className={isCollectionMode ? "w-full md:w-auto h-11" : ""}
            >
              Apply Filters
            </Button>
            <Button
