...
- **Native Ticketing Engine (General Availability - GA)**:
  ...
  - **Legal Consent & Terms Acceptance Architecture**:
    ...
    - **Curated Collections Venue Filter Dropdown & Pre-Aggregated Venues (`enable_venue_filter`)**:
      ...
      - **Backend Pre-Aggregation (`backend/app/api/collections.py` & `backend/app/schemas/collection.py`)**: `GET /api/collections/slug/{slug}` computes and returns `venues: list[VenueSummary]` and `total_venue_count: int` directly on the collection response. Pre-aggregates primary venues (`Event.venue_id`), participating venues (`EventParticipatingVenue`), and custom unlinked `Event.location_name` strings for all published events in the collection, alphabetized without duplicates.
      - **Space-Optimized Dropdown & Hero Stat Counter (`pages/collections/[slug].tsx`)**: Populates the "All Venues" dropdown options and hero stat counter immediately upon initial collection load via `collection.venues` and `collection.total_venue_count`, decoupled from lazy-loaded card pagination. Preserves full dropdown options across infinite scroll and filters in memory (`event.venue_name || event.venue?.name === selectedVenue`) without network requests.
    - **Curated Collections Organizer Profile Filter (`organizer_profile_ids`)**:
      - **Admin Multi-Select (`CollectionsManager.tsx`)**: Searchable multi-select organizer selector in the "Filter Configuration" sidebar. Displays selected count badge, clear button, and filter search input bound to `formData.organizer_profile_ids`. Stored in PostgreSQL `collections.organizer_profile_ids` (`JSONB`, nullable, migration `d4e5f6a7b8c9`) and synced into `filter_params`. Included in the visual preview query.
      - **Backend Event Filtering (`backend/app/api/events.py` & `backend/app/api/collections.py`)**: Supports `organizer_profile_ids` query parameter as comma-separated hex strings and matches both raw and normalized UUIDs (`Event.organizer_profile_id.in_(...)`). In `GET /api/collections/slug/{slug}/events` and `GET /api/events` (including within `city_filter` blocks), applies `organizer_profile_ids` as an absolute root boundary (`.where()`) strictly joined with `AND`, isolating flexible Keyword/Category `OR` conditions inside their own block to prevent event leakage.
      - **Public Collection Page (`pages/collections/[slug].tsx`)**: Passes configured `collection.organizer_profile_ids` and `params.organizer_profile_ids` into `baseFilters` during initial load and infinite scroll pagination.
    - **Curated Collections Slug Validation & Inactive Management**:
      - **Admin Inactive Support (`backend/app/api/collections.py` & `lib/api.ts`)**: Added `include_inactive=True` query parameter to `GET /api/collections`. By default, only active collections are exposed for public pages, while the admin Collections Manager passes `include_inactive: true` so admins can view, manage, and edit inactive or draft collections.
      - **Unique Slug Enforcement (`backend/app/api/collections.py`)**: `create_collection` and `update_collection` explicitly validate slug uniqueness against the database prior to committing and wrap `session.commit()` in `try...except IntegrityError`. Duplicate slugs return a clean HTTP 400 Bad Request error rather than an unhandled 500 UniqueViolation error.
      - **Admin Duplicate Detection (`CollectionsManager.tsx`)**: Warns users in real-time if a slug is already taken by another collection. On submit attempt, alerts the user and renders a 1-click "Edit Existing Collection" shortcut button to seamlessly convert the creation into an update without losing any entered form content.
    - **Curated Collections View & Link Click Analytics Tracking**:
      - **Database & Schemas (`backend/app/models/collection.py`, `backend/app/schemas/collection.py`, `007_add_collection_view_and_click_tracking.sql`)**: Stored in PostgreSQL `collections.view_count` (`INTEGER DEFAULT 0 NOT NULL`) and `collections.link_click_count` (`INTEGER DEFAULT 0 NOT NULL`), with automated startup synchronization via `inline_migrations.py`. Exposed in `Collection` response schema for admin review and list views.
      - **Atomic Tracking Endpoints (`backend/app/api/collections.py`)**: `POST /api/collections/{id_or_slug}/track-view` and `POST /api/collections/{id_or_slug}/track-click` resolve collections by numeric ID or URL slug and execute atomic SQL updates (`values(view_count=Collection.view_count + 1)` and `values(link_click_count=Collection.link_click_count + 1)`), refreshing and returning updated counters.
      - **Client Tracking Integration (`pages/collections/[slug].tsx` & `lib/api.ts`)**: Fires `collectionsAPI.trackView` upon collection load guarded by a `sessionStorage` key (`has_viewed_col_${id}`) to prevent duplicate counts during active sessions. Outbound hero link button triggers non-blocking `navigator.sendBeacon` (falling back to asynchronous API fetch) on click without interrupting standard external navigation (`target="_blank"`).
      - **Admin Analytics Visibility (`CollectionsManager.tsx`)**: Displays real-time view and outbound click metrics both in a dedicated "Stats" column on the admin collections DataTable and within a highlighted analytics banner inside the collection edit modal.
