...
- **Native Ticketing Engine (General Availability - GA)**:
  ...
  - **Legal Consent & Terms Acceptance Architecture**:
    ...
    - **Curated Collections Organizer Profile Filter (`organizer_profile_ids`)**:
      ...
      - **Frontend Client API (`lib/api.ts`)**: Serializes `filters.organizer_profile_ids` into query parameter strings in `eventsAPI.list`, ensuring previews and public collection page requests correctly pass the organizer filter.
